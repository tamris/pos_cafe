...

{
...

    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'transactions.*.open_bill_id' => 'nullable|integer',
        ...
        try {
            ...
            foreach ($offlineList as $offTx) {
                ...
                $realCreatedAt = !empty($offTx['created_at'])
                ...
                if ($openBill && $openBill->status === 'pending') {
                    ...
                    $openBill->save();
                    $openBill->details()->delete();
                    $transaction = $openBill;
                } else {
                    $transaction = Transaction::create([
                        'user_id' => $user->id,
                        'shift_id' => $activeShift?->id,
                ...
                foreach ($offTx['items'] as $item) {
                    ...
                    $detail->created_at = $realCreatedAt;
                    $detail->save();
