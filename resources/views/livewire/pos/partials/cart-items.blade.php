@forelse($cart as $index => $item)
    <div wire:key="cart-item-{{ $index }}-{{ $item['id'] }}" 
         class="p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/80 dark:border-slate-700/80 hover:border-slate-300 dark:hover:border-slate-600 transition-colors space-y-2">
        {{-- Item Header & Stepper --}}
        <div class="flex items-start justify-between gap-2">
            ...
            <div class="flex items-center space-x-1 sm:space-x-1.5 shrink-0">
                ...
                <button type="button" wire:click="updateQuantity({{ $index }}, 'decrease')"
                    ...
                    class="w-7 h-7 flex items-center justify-center bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-slate-700 dark:text-slate-200 transition-colors text-xs font-bold active:scale-95 cursor-pointer">
                ...
                <button type="button" wire:click="removeFromCart({{ $index }})"
                    style="touch-action: manipulation;"
                    class="p-1 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-colors ml-0.5 cursor-pointer"
                    title="Hapus item">
