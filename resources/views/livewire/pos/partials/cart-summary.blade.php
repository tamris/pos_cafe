<div class="space-y-2" x-data="{ showDiscountTax: {{ ($discount > 0 || $tax > 0) ? 'true' : 'false' }} }">
    ...
    <div class="flex items-center justify-between">
        <button type="button" @click="showDiscountTax = !showDiscountTax" 
            class="text-[11px] font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white flex items-center gap-1 transition-colors">
            <span x-text="showDiscountTax ? '▼ Sembunyikan Diskon/Pajak' : '+ Tambah Diskon / Pajak'"></span>
            @if($discount > 0 || $tax > 0)
                <span class="text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-600">
        ...
        <div class="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-900/60 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
            <div>
                ...
                <input type="number" min="0" max="100" wire:model.live="discount"
                    class="w-full px-2 py-1 text-xs font-semibold border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded focus:ring-2 focus:ring-slate-900"
                    placeholder="0">
            </div>
            <div>
                <label class="block text-[10px] font-semibold text-slate-600 dark:text-slate-400 mb-0.5">Pajak (%)</label>
