                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3">
                            ...
                            <div class="relative group w-full sm:w-auto">
                                <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400 group-hover:text-blue-500 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"></path></svg>
                                </div>
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-700 text-xs sm:text-sm">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors {{ $user->trashed() ? 'opacity-70 bg-rose-50/20 dark:bg-rose-950/10' : '' }}">
                                ...
                                <td class="px-5 sm:px-6 py-3.5 whitespace-nowrap text-right space-x-1.5 shrink-0">
                                    ...
                                    @else
                                        {{-- Edit User --}}
                                        <button wire:click="edit({{ $user->id }})" class="inline-flex items-center px-2.5 py-1.5 border border-slate-300 dark:border-slate-600 text-xs font-medium rounded-lg text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors active:scale-95 cursor-pointer">
            ...
            <div class="inline-block bg-white dark:bg-slate-800 rounded-xl text-left overflow-hidden shadow-xl transform transition-all max-w-lg w-full my-8 border border-slate-200 dark:border-slate-700">
                <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}">
                    ...
                    <div class="bg-slate-900 dark:bg-slate-700 text-white px-5 sm:px-6 py-4 border-b border-slate-800 dark:border-slate-600 flex justify-between items-center">
                        ...
                        <button type="button" wire:click="closeModal" class="text-white hover:opacity-80 transition-opacity p-1 cursor-pointer">
                    ...
                    <div class="bg-white dark:bg-slate-800 p-5 sm:p-6 space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input type="password" wire:model="password" placeholder="{{ $isEdit ? 'Kosongkan jika tetap' : 'Min. 6 karakter' }}" class="block w-full px-3 py-2 text-xs sm:text-sm border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-slate-900 dark:focus:ring-blue-500 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1.5">
                                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase">PIN Apps <span class="text-rose-500">*</span></label>
                                    <button type="button" wire:click="generateRandomPin" class="text-[11px] text-blue-600 dark:text-blue-400 hover:underline font-bold cursor-pointer">Acak PIN</button>
                                </div>
                                <input type="text" wire:model="pin" maxlength="6" placeholder="6 Digit (123456)" class="block w-full px-3 py-2 text-xs sm:text-sm font-mono tracking-widest border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-slate-900 dark:focus:ring-blue-500 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400">
                                @error('pin') <span class="text-rose-500 dark:text-rose-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative group">
                                ...
                                <select wire:model="role" class="appearance-none block w-full pl-10 pr-10 py-2 text-xs sm:text-sm font-medium border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-900 text-slate-900 dark:text-white cursor-pointer transition-all outline-none">
                                    <option value="kasir">Kasir (Staf)</option>
                                    <option value="admin">Administrator (Pemilik)</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center pr-3.5 pointer-events-none text-slate-400">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-700/50 px-5 sm:px-6 py-3.5 flex justify-end space-x-2.5 border-t border-slate-200 dark:border-slate-700">
                        <button type="button" wire:click="closeModal" class="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition-colors font-medium text-xs cursor-pointer">
                            Batal
                        </button>
                        <button type="submit" class="px-4 py-2 bg-slate-900 dark:bg-blue-600 text-white rounded-lg hover:bg-slate-800 dark:hover:bg-blue-700 transition-colors font-semibold text-xs shadow-sm active:scale-95 cursor-pointer">
